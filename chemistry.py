...
SUBSTITUTES = ["[OH]", "[NH2]", "[CH3]", "F", "Br", "c1ccccc1", "C(=O)O", "C(=O)N", "S(=O)(=O)N", "C#N"]
...
def calculate_properties(mol):
    ...
    if mol:
        ...
        logS = -0.01 * mol_weight - logp  # Solubility prediction based on molecular weight and LogP
        return {
            "Molecular Weight": mol_weight,
            "Solubility (LogS)": logS,
            "Permeability (LogP)": logp  # Add permeability as LogP
        }
    return {"Molecular Weight": None, "Solubility (LogS)": None, "Permeability (LogP)": None}
...
def substitute_and_evaluate(args):
    ...
    try:
        ...
        mol_smiles_cleaned = mol_smiles.replace("*", "")  # Remove the attachment point marker
